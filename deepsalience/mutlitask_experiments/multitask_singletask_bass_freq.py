...
def get_model():
    ...
    y_bass_feat = Conv2D(
        32, (3, 3), padding='same', activation='relu', name='bass_filters')(y_concat_freq) #32
    ...
    y_bass_feat4 = Conv2D(
        16, (7, 7), padding='same', activation='relu', name='bass_filters4')(ya_bass_feat2) #16
    ...
    model = Model(inputs=[y0, y_freq], outputs=[bass])
...
model = get_model()
output_path = '../experiment_output/multitask_singletask_bass_freq'
tasks = ['bass']
data_types = None
